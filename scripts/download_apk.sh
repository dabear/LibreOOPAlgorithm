...
apkSystemLocation="$HOME/LibreLink_latest_apkpure.com.apk"
...
function runit(){
  ...
  if [ "x${#apk}" == "x0" ] ; then
    doexit "Could not download local LibreLink apk in $HOME. Please manually download LibreLink_v1.3.2.4_apkpure.com.apk (or newer) and place it in $HOME"
  else
